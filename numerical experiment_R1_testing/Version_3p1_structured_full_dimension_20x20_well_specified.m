function Version_3p1_structured_full_dimension_20x20_well_specified()
%==========================================================================
% 20x20 Version 3.1 (well-specified linear contextual model, ambient full baseline)
%
% Goal:
...
%   (3) Stage II now reports TWO full baselines:
%         - centered/clipped full SPO+ (diagnostic; tends to be implicitly
%           compressed into W* on this corridor family), and
%         - ambient affine full SPO+ with FREE intercept and NO clipping,
%           started from zero. This baseline can move in all d directions,
%           so it pays the ambient-dimensional burden from the start.
%
% IMPORTANT:
...
fprintf('=== Version 3.1 (well-specified), 20x20 structured full-dimensional experiment ===\n');
...
fprintf('\n=== Version 3.1 (well-specified) compression summary ===\n');
...
% -------------------------------- Plots ----------------------------------
figRisk = figure('Name', 'Version 3.1 (well-specified): Test SPO risk');
...
legend({'Full SPO+ (centered/clipped)', ...
        ...
        'Reduced SPO+ after learned W', ...
        'Reduced SPO+ with oracle W*'}, ...
    'Location', 'best');
title(sprintf('Version 3.1 well-specified, g=%d, d=%d, true d_*=%d, affdim(C)=d', cfg.g, d, rstar));

figDim = figure('Name', 'Version 3.1 (well-specified): learned dimension');
hold on; grid on; box on;
...
figAlign = figure('Name', 'Version 3.1 (well-specified): alignment with true W*');
hold on; grid on; box on;
...
resultsDir = prepare_results_dir_version3p1_well_specified();
save_figure(figRisk,  fullfile(resultsDir, 'version3p1_well_specified_spo_risk.png'));
save_figure(figDim,   fullfile(resultsDir, 'version3p1_well_specified_dimW.png'));
save_figure(figAlign, fullfile(resultsDir, 'version3p1_well_specified_alignment.png'));
save(fullfile(resultsDir, 'version3p1_well_specified_summary.mat'), ...
    'cfg', 'riskFullCentered', 'riskFullAmbient', 'riskLearn', 'riskOracle', ...
    'dimW', 'capTrue', 'capHat', 'maxAngDeg', 'numHard', ...
    'mFC', 'ciFC', 'mFA', 'ciFA', 'mL', 'ciL', 'mO', 'ciO', ...
...
function resultsDir = prepare_results_dir_version3p1_well_specified()
repoDir = fileparts(mfilename('fullpath'));
resultsDir = fullfile(repoDir, 'results_version3p1_well_specified');
