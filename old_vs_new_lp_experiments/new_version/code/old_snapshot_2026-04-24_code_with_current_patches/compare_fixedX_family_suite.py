...
def make_shortest_path_problem(args: argparse.Namespace, rng: np.random.Generator) -> OriginalFixedXProblem:
    if str(getattr(args, "shortest_design", "")).lower() == "corridor":
        return make_corridor_shortest_path_problem(args, rng)
...
def monotone_connector_edges(
    ...
    """Right/down connector from one grid cell to another."""
    if r1 < r0 or c1 < c0:
        raise ValueError("connector end point must dominate start point")
    out: List[int] = []
    r, c = int(r0), int(c0)
    while r < r1 or c < c1:
        u = r * grid_size + c
...
def make_corridor_shortest_path_problem(args: argparse.Namespace, rng: np.random.Generator) -> OriginalFixedXProblem:
    """Shortest-path instance with independent 2x2 corridor gadgets."""
    del rng
    grid_size = int(args.grid_size)
    edges, edge_to_idx = make_monotone_grid_edges(grid_size)
    n_edges = len(edges)
    ...
    dstar = int(getattr(args, "shortest_gadgets", 0))
    if dstar <= 0:
        dstar = min(max(4, int(getattr(args, "cost_rank", 12))), (grid_size - 1) // 2)
    dstar = min(dstar, (grid_size - 1) // 2)

    low_base = float(getattr(args, "sp_cost_low", 10.0))
    high_base = float(getattr(args, "sp_cost_high", 100.0))
    costs0 = np.full(n_edges, high_base, dtype=float)
    corridor_mask = np.zeros(n_edges, dtype=bool)
    U_reward_raw = np.zeros((n_edges, dstar), dtype=float)
    gadgets: List[Tuple[int, int, int, int]] = []
    square_tl: List[Tuple[int, int]] = []
    anchor_edges: List[int] = []

    cur_r = 0
    cur_c = 0
    for k in range(dstar):
        r = 2 * k
        c = 2 * k
        square_tl.append((r, c))
        conn = monotone_connector_edges(cur_r, cur_c, r, c, grid_size, edge_to_idx)
        corridor_mask[np.asarray(conn, dtype=int)] = True
        anchor_edges.extend(conn)

        e_top = edge_to_idx[(r * grid_size + c, r * grid_size + c + 1)]
        e_right = edge_to_idx[(r * grid_size + c + 1, (r + 1) * grid_size + c + 1)]
        e_left = edge_to_idx[(r * grid_size + c, (r + 1) * grid_size + c)]
        e_bottom = edge_to_idx[((r + 1) * grid_size + c, (r + 1) * grid_size + c + 1)]
        sq_edges = [e_top, e_right, e_left, e_bottom]
        corridor_mask[np.asarray(sq_edges, dtype=int)] = True
        gadgets.append((e_top, e_right, e_left, e_bottom))
        anchor_edges.extend([e_top, e_right])
    ...
    tail_conn = monotone_connector_edges(cur_r, cur_c, grid_size - 1, grid_size - 1, grid_size, edge_to_idx)
    corridor_mask[np.asarray(tail_conn, dtype=int)] = True
    anchor_edges.extend(tail_conn)
    costs0[corridor_mask] = low_base

    reward_center = reward_from_costs_via_potential(edges, costs0, n_nodes, scale=float(getattr(args, "sp_potential_scale", 6.0)))
    U_reward = orthonormalize_columns(U_reward_raw)
    anchor_x0 = np.zeros(n_edges, dtype=float)
    anchor_x0[np.asarray(anchor_edges, dtype=int)] = 1.0
    return OriginalFixedXProblem(
        name="shortest_path_corridor_fixedX",
        reward_center=reward_center,
        A_ineq=np.zeros((0, n_edges), dtype=float),
        b_ineq=np.zeros(0, dtype=float),
        A_eq=B,
        ...
        metadata={
            ...
            "demand": demand,
            "gadgets": np.asarray(gadgets, dtype=int),
            "corridor_edges": np.flatnonzero(corridor_mask),
            "base_costs": costs0,
            "grid_size": np.asarray([grid_size], dtype=int),
            "square_tl": np.asarray(square_tl, dtype=int),
            "anchor_x0": anchor_x0,
            "shortest_design": np.asarray(["corridor"], dtype=object),
        },
    )
